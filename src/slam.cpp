...
int main( int argc, char** argv )
{
    ...
    *******************************/
    ...
    globalOptimizer.optimize( 10000 ); //可以指定优化步数
    ...
    for (size_t i=0; i<keyframes.size(); i++)
    {
        ...
        for (int i = 0;i < tmp->points.size();++i){
            cloud_octo.push_back( tmp->points[i].x, tmp->points[i].y, tmp->points[i].z);
        }
        ...
        for (int i = 0;i < tmp->points.size();++i)
            tree.integrateNodeColor( tmp->points[i].x, tmp->points[i].y, tmp->points[i].z, tmp->points[i].r, tmp->points[i].g, tmp->points[i].b);
        *output += *tmp;
        tmp->clear();
        newCloud->clear();
    }
...
CHECK_RESULT checkKeyframes( FRAME& f1, FRAME& f2, g2o::SparseOptimizer& opti, bool is_loops)
{
    ...
    /*PointCloud::Ptr f1c = image2PointCloud( f1.rgb, f1.depth, camera );
    PointCloud::Ptr f2c = image2PointCloud( f2.rgb, f2.depth, camera );
    PointCloud::Ptr out(new PointCloud());
    pcl::IterativeClosestPoint<PointT, PointT> icp;
    icp.setMaximumIterations(25);
    icp.setInputSource(f1c);
    icp.setInputTarget(f2c);
    icp.align(*out);
    if(icp.hasConverged()){
        Eigen::Isometry3d T;
        Eigen::Matrix4d AT = icp.getFinalTransformation().cast<double>();
        for(int i=0; i<4;++i){
            for(int j=0; j<4; ++j){
                T(i,j) = AT(i,j);
            }
